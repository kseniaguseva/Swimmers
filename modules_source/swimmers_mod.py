...
def move_bac(t, x, ux, uy, ux_p, uy_p, ug, grid, swimmer, N_bac, step, dt, tor):
    dtheta = []
    dx = []
    dy = []
    ...
    ux_u = shift_frame(ug, t, grid, ux_u)
    uy_u = shift_frame(ug, t, grid, uy_u)   
    ...
    for i in range(0, N_bac):
        ...
        #dtheta.append(swimmer.alpha*(scos[xn,yn]*cos(2*theta) - ssin[xn,yn]*sin(2*theta)))
...
def swimmer_integrate(dt, i, bac, ux, uy, ux_p, uy_p, ug, grid, swimmer, solver, tor = None):
...
def swimmer_integrate_periodic(dt, i, bac, ux, uy, ux_p, uy_p, ug, grid,
     ...
     if (rw == True):
          ...
          for aux in range(bac.N_bac):
               if(event[aux] < dt/swimmer.tau_run):
                    bac.theta[aux] +=  swimmer.phi0
    
     bac.theta = bac.theta%(2*pi)
