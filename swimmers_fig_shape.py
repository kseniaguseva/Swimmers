...
swimmers_rE = Swimmer(uf = randflow.u, fr = 1., alpha = 0.75, phi0 = pi)
bac_rE = Ensemble(grid, N_bac = 500, chemo = True)

swimmers_rR = Swimmer(uf = randflow.u, fr = 1., alpha = 0., phi0 = pi)
...
for i in range(time):
   ...
   for j in range(0, bac_rE.N_bac):
        chemotaxis(bac_rE, C, grid, i*dt, dt, chemo, swimmers_rE, j)
   for j in range(0, bac_rR.N_bac):
        chemotaxis(bac_rR, C, grid, i*dt, dt, chemo, swimmers_rR, j)
            
   swimmer_integrate(dt, i, bac_rE, ux, uy, ux_p, uy_p, ug, grid, swimmers_rE, solver)
   ...
   randflow.flow_evolve(dt, grid)
   ux, uy = ux_p, uy_p
   print(i*dt, bac_rE.N_bac, bac_rR.N_bac)
        


   if (i*dt > 40.):
        if(i%50 == 0):
          filename = "./Figures_shape/swim_%.3f" %(i*dt)
          ...
          for j in range(0, bac_rR.N_bac):
               patch = patches.Ellipse([bac_rR.x[j]/grid.delta, bac_rR.y[j]/grid.delta], width = 1.5,  height = 1.5,angle = (bac_rR.theta[j]/pi)*180, color = "black")
               patch.update({'facecolor': 'white'})
               
               ax1.add_patch(patch)
          for j in range(0, bac_rE.N_bac):
               patch = patches.Ellipse([bac_rE.x[j]/grid.delta, bac_rE.y[j]/grid.delta], width = 3.5,  height = 1.2,angle = (bac_rE.theta[j]/pi)*180, color = "black")
               patch.update({'facecolor': 'white', 'linewidth': 0.5 })
               ax2.add_patch(patch)
